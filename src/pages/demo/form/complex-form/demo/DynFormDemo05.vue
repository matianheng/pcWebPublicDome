<template>
  <div class="flex flex-col">
    ...
    <div class="mb-3">加载到的必填字段: {{ fieldList }}</div>
    <LsyDynForm
      ref="dynFormRef"
...
defineOptions({ name: 'DynFormDemo05' })
...
function onLoadBackendData() {
  fieldList.value.push(...['isChina', 'name'])
}
const formItemArr = ref<LsyFormItemConfig[]>([
  {
    ...
    prop: 'name',
    rules: () => [
      { required: fieldList.value.includes('name'), message: '该项必填' },
    ],
  },
  {
    name: 'ElSelect',
    ...
    rules: () => [
      { required: fieldList.value.includes('isChina'), message: '该项必填' },
    ],
  },
  {
    name: 'ElSelect',
