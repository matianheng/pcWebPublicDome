<template>
  <div class="h-full w-full relative overflow-hidden">
    <!-- sticky元素 -->
    <div
      v-if="stickyClassifyComp !== undefined"
      ref="stickyClassifyDomRef"
      class="absolute top-0 z-[3] js-sticky-dom-width"
    >
      <!-- <component :is="stickyClassifyComp" :classify="stickyClassify"></component> -->
      <component
        :is="classifyComp"
        :classify-info="classifyMap.get(stickyClassifyComp)"
      ></component>
    </div>
 ...
 * 包含距离滚动容器顶部距离的分类信息
 */
interface HasPositionClassifyInfo {
  /**
   * 距离滚动容器顶部距离
   */
  offsetTop: number
 ...
 * 有位置信息的分类数据
 */
const hasPositionClassifyArr = ref<HasPositionClassifyInfo[]>([])
/**
 * 当前激活的分类
 */
...
const stickyClassifyDomRef = ref<HTMLDivElement>()
/**
 * 计算竖向滚动条宽度
 */
...
async function searchData(next: boolean, initClassify?: number | string) {
  ...
  if (next) {
    ...
    const newHasPositionClassifyArr: HasPositionClassifyInfo[] = []
    let arrPos = 0
    ret.list.forEach((item, idx) => {
    ...
    const tmpList = hasPositionClassifyArr.value
    tmpList.forEach((item, idx) => {
      // const prevIdx = idx - 1
      // let prevHasPositionClassify: HasPositionClassifyInfo | undefined
      // if (prevIdx < 0) {
      //   if (newHasPositionClassifyArr.length > 0) {
      //     prevHasPositionClassify =
      //       newHasPositionClassifyArr[newHasPositionClassifyArr.length - 1]
      //   }
      // } else {
      //   prevHasPositionClassify = newHasPositionClassifyArr[prevIdx]
      // }
      // item.offsetTop = prevHasPositionClassify
      //   ? prevHasPositionClassify.offsetTop + prevHasPositionClassify.height
      //   : 0
      newHasPositionClassifyArr.push(item)
    })
    hasPositionClassifyArr.value = newHasPositionClassifyArr
...
function appendClassifyItem(
  ...
  dataArr: ItemData[],
  dataPositionArr: DataPositionHeight[],
  tmpHasPositionClassifyArr: HasPositionClassifyInfo[]
) {
  const dataType: ItemDataType = 2
  ...
  */
  ...
  const classifyInfo = classifyMap.value.get(item.classify)
  if (classifyInfo) {
    tmpHasPositionClassifyArr.push({ offsetTop: startPx, classifyInfo, height })
  }
  arrPos += 1
  return arrPos
}
...
const findStickClassifyThrottle = throttle(findStickClassifyWrapper, 100)
/**
 * 根据滚动位置，实时计算视口显示数据
 * @param evt
 */
...
function updateStickyDomPosition(scrollTop: number) {
  const stickyDom = stickyClassifyDomRef.value
  if (!stickyDom) return

  const stickyClassify = stickyClassifyComp.value
  if (stickyClassify === undefined) return

  const nextStickyClassify = findNextStickyClassify(stickyClassify)
  if (nextStickyClassify === undefined) return

  // 找下个分类元素的位置信息
  const nextClassifyPosition = findClassifyPosition(nextStickyClassify)

  // 找当前分类元素的位置信息
  const currentClassifyPosition = findClassifyPosition(stickyClassify)
  if (!currentClassifyPosition) {
    if (nextClassifyPosition) {
      // 当前分类元素的位置信息未找到，但找到了下一个分类元素的位置信息(当前的状况应该为:点击了某个分类数据，然后正在向上滑动)
      // 以下一个分类元素的offsetTop和height为基准, 计算滚动阈值
      const flagScrollTop =
        nextClassifyPosition.offsetTop - nextClassifyPosition.height
      doUpdateStickyDomPosition(
        scrollTop,
        flagScrollTop,
        stickyDom,
        nextClassifyPosition.height
      )
    }
    return
  }
  ...
  if (!nextClassifyPosition) {
    // 未找到下个分类的位置信息，则sticky元素固定在最顶部
    stickyDom.style.top = `0`
  } else {
    // 找到了下个分类的位置信息
    // 计算滚动阈值，scrollTop不超过该阈值，则stickyDom.style.top为0，反之，则需要将stickyDom.style.top设置为 (scrollTop-flagScrollTop)
    const flagScrollTop =
      nextClassifyPosition.offsetTop - currentClassifyPosition.height
    doUpdateStickyDomPosition(
      scrollTop,
      flagScrollTop,
      stickyDom,
      currentClassifyPosition.height
    )
  }
}
...
/**
 * 更新sticky元素的位置
 * 1. 如果当前滚动条的位置未超过阈值，则sticky元素保持浮动在最顶部
 * 2. 如果当前滚动条的位置已超过阈值，则计算两者之间的差值，并将sticky元素的top值设置为该差值
 *
 * @param scrollTop 滚动条位置
 * @param flagScrollTop 滚动阈值
 * @param stickyDom sticky元素
 */
function doUpdateStickyDomPosition(
  scrollTop: number,
  flagScrollTop: number,
...
function findClassifyPosition(classify: number | string) {
  const arr = hasPositionClassifyArr.value
  if (!arr || arr.length === 0) return

  return arr.find(item => item.classifyInfo.id === classify)
}

function findNextStickyClassify(stickyClassify: number | string) {
  const idx = classifyArr.value.findIndex(item => item.id === stickyClassify)
  if (idx >= 0 && idx < classifyArr.value.length - 1) {
    return classifyArr.value[idx + 1].id
  }
  return undefined
}
...
function doUpdateDataPosition(contentWrapperDom: HTMLElement) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    // 获取dom元素在allDataPositionArr中的数组下标
    ...
    if (height !== item.height) {
      ...
      let itemData = allDataArr[index]
      if (itemData.dataType === 2) {
        updateClassifyPosition(height, itemData.customData.classify)
      }
      let prevIndex = index
      for (let j = index + 1; j < allDataPositionArr.length; j++) {
        ...
        itemData = allDataArr[j]
        if (itemData.dataType === 2) {
          updateClassifyPosition(
            curItem.height,
            itemData.customData.classify,
            curItem.startPx
          )
        }
      }
...
function updateClassifyPosition(
  height: number,
  classify: number | string,
  offsetTop?: number
) {
  const tmpArr = hasPositionClassifyArr.value
  for (let i = 0; i < tmpArr.length; i++) {
    if (tmpArr[i].classifyInfo.id === classify) {
      tmpArr[i].height = height
      if (undefined !== offsetTop) {
        tmpArr[i].offsetTop = offsetTop
      }
      break
    }
  }
